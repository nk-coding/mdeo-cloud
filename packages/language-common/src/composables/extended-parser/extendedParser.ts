 ...
 */
export function generateExtendedParser(
    context: PluginContext,
    ruleOverrride: RuleOverride
): {
    ...
    class ExtendedLangiumParser extends context.langium.LangiumParser implements ExtendedParser {
    ...
    class ExtendedCompletionParser extends context.langium.LangiumCompletionParser implements ExtendedParser {
        get chevrotainParser(): EmbeddedActionsParser {
            return this.wrapper;
        }
    ...
    return {
        LangiumParser: (services) => {
            ...
            const parser = context.langium.createParser(grammar, new ExtendedLangiumParser(services), lexer.definition);
            parser.finalize();
            return parser;
        },
        CompletionParser: (services) => {
            const grammar = services.Grammar;
            const lexer = services.parser.Lexer;
            const parser = context.langium.createParser(
                grammar,
                new ExtendedCompletionParser(services),
                lexer.definition
            );
            parser.finalize();
            return parser;
