...
export class BrowserFileSystem extends FileSystem {
  ...
  async initialize(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        this.ensureRootFolder().then(() => resolve()).catch(reject);
      };
  ...
  async createFile(options: CreateFileOptions): Promise<File> {
    ...
    const parentId = options.parentId || this.rootFolderId;
  ...
  async moveNode(id: string, options: MoveOptions): Promise<FileSystemNode> {
    ...
    const targetParentId = options.targetParentId || this.rootFolderId;
