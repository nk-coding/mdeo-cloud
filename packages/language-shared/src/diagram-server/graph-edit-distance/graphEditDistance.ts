...
export function* optimizeEditPaths(
  ...
  let pendingU = [...G1.nodes];
  let pendingV = [...G2.nodes];
  ...
  if (nodeSubstCost) {
    for (let i = 0; i < mV; i++) {
      for (let j = 0; j < nV; j++) {
        CV[i][j] = nodeSubstCost(
          G1.getNodeData(pendingU[i]),
          G2.getNodeData(pendingV[j])
        );
      }
    }
  }
  ...
  if (edgeSubstCost) {
    for (let i = 0; i < mE; i++) {
      ...
      for (let j = 0; j < nE; j++) {
        const h = pendingH[j];
        CE[i][j] = edgeSubstCost(
          G1.getEdgeData(g[0], g[1], g[2]),
          G2.getEdgeData(h[0], h[1], h[2])
        );
      }
    }
  }
  ...
  let Ce = makeCostMatrix(CE, mE, nE);
  ...
  function* getEditOps(
    ...
    for (const [candI, candJ] of candidates) {
      ...
      other.push([
        ...
        xyCand,
        CeXyCand,
        CvMatrix.C[candI][candJ] + localCeCand.ls,
      ]);
    }
  ...
  function* getEditPaths(
    ...
    for (const [ij, CvIj, xy, CeXy, editCost] of editOps) {
      ...
      for (const [x, y] of xy) {
        matchedGh.push([
          x < lenG ? pendingGList[x] : null,
          y < lenH ? pendingHList[y] : null,
        ]);
      }
